...
import { Card, Eyebrow } from "@aishka/ui/primitives";
...
export function Build() {
  useTitle("Your build · AIshka");
  const build = useLoaderData() as BuildView;
          ...
          return (
            <section key={stage.id}>
              <div className="mb-3 flex items-center gap-3">
                <span aria-hidden className="flex h-7 w-7 flex-none items-center justify-center">
                  <Emoji symbol={stage.icon ?? "▫️"} className="h-6 w-6 object-contain" fallbackSize="text-[22px]" />
                </span>
              ...
              <div className="grid gap-2.5">
                {stage.slots.map((slot) => (
                  <SlotCard key={slot.id} slot={slot} open={build.next?.id === slot.id} />
                ))}
              </div>
